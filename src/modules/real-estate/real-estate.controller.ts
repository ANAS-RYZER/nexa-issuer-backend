import { Body, Controller, Get, Post, Put, Param, HttpCode, HttpStatus, UseGuards, Req, Query } from "@nestjs/common";
...
export class AssetController {
    ...
    async getPublicAssetById(
        ...
        const asset = await this.assetService.getPublicAssetById(assetId, effectiveInvestorId);
    ...
    */
    ...
    @HttpCode(HttpStatus.OK)
    async getAdminAssetListing(@Query() query: AdminAssetListingQueryDto, @Req() req: Request) {
        const issuerId = (req as any).user?.userId;
        console.log("Issuer ID in Controller:", issuerId);
        const result = await this.assetService.getAdminAssetListing(query, issuerId);
    ...
    */
    ...
    async createAsset(@Body() body: CreateAssetDto, @Req() req: Request) {
        ...
        const newAsset = await this.assetService.createAssetBasicDetails(body, adminId);
    ...
    async getAssetById(
        ...
        const issuerId = (req as any).user?.userId;
        const asset = await this.assetService.getAssetByIdWithDetails(assetId, issuerId);
